...
uint16_t g_msec_copy = 0;
volatile uint16_t g_msec_temp = 0;
...
int main() {
    ...
    // Optional extra pin metadata
    bi_decl(bi_1pin_with_name(21, "WIZNET INTn"));
    bi_decl(bi_1pin_with_name(24, "VBUS sense"));
    bi_decl(bi_1pin_with_name(25, "User LED"));
    bi_decl(bi_1pin_with_name(29, "VSYS/3 ADC"));
    bi_decl(bi_program_description("WIZnet Loopback Example for W6100-EVB-Pico2"));     // info -a wizchip_loopback.elf
    // to see call function:
    // ~/project/pico2/work/WIZnet-PICO-C/build/examples/loopback$ ~/.pico-sdk/picotool/2.2.0/picotool/picotool info --pins wizchip_loopback.elf
...
static void repeating_timer_callback(void) {
    ...
    g_msec_copy = g_msec_cnt;
    g_msec_temp = g_msec_cnt;
