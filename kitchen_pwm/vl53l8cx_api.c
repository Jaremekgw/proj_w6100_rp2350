		...
		VL53L8CX_Configuration		*p_dev)
{
	...
	printf("[VL53] api init L:257 status:%u addr:%x\n", status, p_dev->platform.i2c_addr);

	/* SW reboot sequence */
	status |= WrByte(&(p_dev->platform), 0x7fff, 0x00);
	printf("[VL53] api init first byte write status:%u\n", status);
	...
	printf("[VL53] api init - first wait status:%u\n", status);
	...
	printf("[VL53] api init - L:369  status:%u\n", status);
