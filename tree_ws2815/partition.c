...
int get_alt_part_addr(boot_info_t *boot_info, partition_info_t *part_info) {
    ...
    int pt_id = get_corresponding_partition_id((unsigned int)boot_info->partition);
...
bool read_next_partition(pico_partition_table_t *pt, pico_partition_t *p) {
    ...
    if (p->extra_family_id_count | p->has_name) {
        ...
        uint32_t arg = ((uint32_t)pt->current_partition << 24) | flags;
        int rc = rom_get_partition_table_info(extra_family_ids_and_name, sizeof(extra_family_ids_and_name), arg);
        if (rc < 0) {
            pt->status = rc;
            return false;
        }
...
static int get_single_partition_info(int partition_id, partition_info_t *info) {
    ...
    first_sector_number = (uint16_t)((out[1] & PICOBIN_PARTITION_LOCATION_FIRST_SECTOR_BITS) >> PICOBIN_PARTITION_LOCATION_FIRST_SECTOR_LSB);
    last_sector_number = (uint16_t)((out[1] & PICOBIN_PARTITION_LOCATION_LAST_SECTOR_BITS) >> PICOBIN_PARTITION_LOCATION_LAST_SECTOR_LSB);
